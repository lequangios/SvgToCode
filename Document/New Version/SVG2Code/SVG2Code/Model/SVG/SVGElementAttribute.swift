...
extension Array where Element == SVGElementAttribute {
    func getLastStyle()->SVGElementAttribute {
        let list = self.sorted(by: {$0.priority < $1.priority})
        var shapeStyle = list.first ?? SVGElementAttribute()
        for item in list {
