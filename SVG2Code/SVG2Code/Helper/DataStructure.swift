...
//MARK:--
//MARK: Stack
public class Stack<T:NodeProtocol>{
    private var list:LinkedList<T>
    
    var isEmpty:Bool { return list.isEmpty }
    var isFull:Bool { return list.isFull }
    var size:Int { return list.getSize() }
    var length:Int { return list.getLength()}
    
    init() {
        list = LinkedList<T>.init()
    }
    
    init(maxSize: Int) {
        list = LinkedList<T>.init(maxSize: maxSize)
    }
    
    func push(node:Node<T>) -> ErrorCode {
        return list.insertHead(node: node)
    }
...
//MARK:--
//MARK: Queue
public class Queue<T:NodeProtocol>{
    ...
    func enQueue(data:T) -> ErrorCode {
        let node = Node<T>.init(data: data)
        return list.insertTail(node: node)
    }
